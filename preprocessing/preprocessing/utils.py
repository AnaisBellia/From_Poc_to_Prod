...
#### PB de shape left (0,5) et right (1,5) du pipe
    ...
    def get_train_batch(self):
        ...
        # on embed new_x puisque c'est le texte en string, new_y est le label
        new_x = self.preprocess_text(self.x_train[i*self.batch_size:(i+1)*self.batch_size])
        # TODO: CODE HERE
        # takes y_train between i * batch_size to (i + 1) * batch_size
        new_y =self.y_train[i*self.batch_size:(i+1)*self.batch_size]
    ...
    def get_test_batch(self):
        ...
        # TODO: CODE HERE
        # takes x_train between i * batch_size to (i + 1) * batch_size, and apply preprocess_text
        new_x = self.preprocess_text(self.x_test[i*self.batch_size:(i+1)*self.batch_size])
        # TODO: CODE HERE
        # takes y_train between i * batch_size to (i + 1) * batch_size
        new_y =self.y_test[i*self.batch_size:(i+1)*self.batch_size]
        # When we reach the max num batches, we start anew
        # recupere le reste
